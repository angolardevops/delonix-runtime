...
confinada ao `-bin` e, desde a P2c do ADR-0040, ao `delonix-compute` — os tipos de spec de Pod derivam o schema ao lado da sua definição, que é o que impede o schema publicado de divergir deles; o ADR-0040 põe o schema gerado na camada de contextos. Os crates de mecanismo continuam dep-limpos (`cargo tree -e normal -p <crate>`: nem `schemars` nem `ratatui` aparecem em nenhum adapter ou foundation). O schema é tão estrito quanto
