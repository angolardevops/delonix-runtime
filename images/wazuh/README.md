# Wazuh (SIEM/XDR) — build a qcow2 image

This folder builds Wazuh, the SIEM/XDR platform, installed on Ubuntu. It is a cloud image with the software installed, and its first boot finishes the setup.
...
**Note.** The first boot generates its own credentials and certificates, so every deployed VM has its own.
...
WAZUH_VERSION=X.Y.Z delonix vm build -f images/wazuh/vm.yaml
```
...
the top of `scripts/appliances/build-wazuh.sh`.
...
```bash
./scripts/appliances/verify-wazuh.sh
```

## 5. Boot a VM from it

```bash
delonix vm create demo --disk wazuh-4.14.7-1-r1 --wait
```
...
- **`no builder 'wazuh'`** — you are not inside a checkout of the
  repository, or `scripts/appliances/build-wazuh.sh` is missing.
