...
mod tests {
    ...
    /// The parallel walk HAS to give the same number as the sequential one — it
    /// is this number that enforces the rootless quota, the only enforcement
    /// rootless has.
    ///
    /// The tree is built to exercise exactly where the two could diverge:
    /// hardlinks seen by DIFFERENT workers (the same inode reachable by several
    /// names spread over distinct subtrees), a DEEP tree (where splitting the
    ...
    fn the_parallel_walk_gives_exactly_what_the_sequential_one_does() {
        ...
        for i in 0..40 {
            let d = root.join(format!("wide{i}"));
            std::fs::create_dir_all(&d).unwrap();
            std::fs::write(d.join("f"), vec![b'x'; 4096 + i]).unwrap();
        }
        // deep: a single entry at the top, everything below it
        let mut deep = root.join("deep");
        for i in 0..12 {
            deep = deep.join(format!("n{i}"));
            std::fs::create_dir_all(&deep).unwrap();
            std::fs::write(deep.join("f"), vec![b'y'; 8192]).unwrap();
        }
        // hardlinks: one inode, four names, in different subtrees — the case a
        // wrong merge counts four times.
        let target = root.join("wide0").join("linked");
        std::fs::write(&target, vec![b'z'; 65536]).unwrap();
        for i in 1..4 {
            std::fs::hard_link(&target, root.join(format!("wide{i}")).join("linked")).unwrap();
        }

        let seq = super::dir_usage_parallel(root, 1);
        for t in [2usize, 4, 8] {
            let par = super::dir_usage_parallel(root, t);
            assert_eq!(
                par.bytes, seq.bytes,
                "{t} thread(s) gave a total different from the sequential one"
            );
            assert_eq!(par.unreadable, seq.unreadable, "{t} thread(s)");
        }

        // And the deduplication is REAL, not an accident of the tree being small:
        // the four names are worth a single file.
        let no_link = super::dir_usage_parallel(&root.join("deep"), 8);
        assert!(no_link.bytes > 0);
        assert!(
