...
mod tests {
    ...
    #[test]
    fn only_a_vanished_root_counts_as_orphaned() {
        let atual = Path::new("/home/w/.local/share/delonix");
        let other_root = Path::new("/tmp/delonix-itest-1-0");

        // The current root is NEVER orphaned — even if `exists` were to say otherwise.
        assert!(!classify_stray(Some(atual), atual, true));
        assert!(!classify_stray(Some(atual), atual, false));

        // Another LIVE root belongs to another engine: hands off.
        assert!(!classify_stray(Some(other_root), atual, true));

        // Another DELETED root: this is the case, and the only one.
        assert!(classify_stray(Some(other_root), atual, false));

        // An unreadable environment is not proof of abandonment.
        assert!(!classify_stray(None, atual, false));
    }

    /// The slirp's ownership token is the PATH of the api-socket, and the boundary
    /// has to be exact: a raw `starts_with` would accept `delonix-net-10001` for uid
    /// 1000 — another user's slirp, on a shared host.
    #[test]
    fn the_slirp_socket_identifies_the_owner_without_catching_the_neighbouring_uid() {
    ...
    fn the_api_socket_is_read_in_both_spellings() {
        ...
        let without: Vec<String> = ["slirp4netns", "1", "tap0"]
            .iter()
            .map(|s| s.to_string())
            .collect();
        assert_eq!(slirp_api_socket(&without), None);
    }

    /// The target is POSITIONAL and its position depends on the flags before it.
    /// A fixed index would read a flag as the pid the day one was added.
    #[test]
    fn the_slirp_target_is_read_from_the_positionals_and_not_from_an_index() {
        let argv: Vec<String> = [
            "slirp4netns",
            "--configure",
            "--mtu=65520",
        ...
        // A flag added in front does not shift the answer.
        let mut com_flag = argv.clone();
        com_flag.insert(1, "--enable-sandbox".into());
        assert_eq!(slirp_target_pid(&com_flag), Some(716912));

        // Without enough positionals no pid is invented.
        let short: Vec<String> = ["slirp4netns", "--configure"]
            .iter()
            .map(|s| s.to_string())
            .collect();
        assert_eq!(slirp_target_pid(&short), None);
    }
}
