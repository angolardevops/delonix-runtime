...
pub fn libvirt_report(host: &LibvirtHost) -> ProviderReport {
    ...
    ProviderReport::build(
        ...
        C::VmResizeCold => bin(S::Supported { evidence: "check:o domínio arranca com 2 vCPU" }),
...
pub fn cloud_hypervisor_report(host: &CloudHypervisorHost) -> ProviderReport {
    ...
    ProviderReport::build(
        ...
        C::VmClone => S::NotImplemented,
        C::VmTemplate => S::NotImplemented,
        C::VmResizeCold => bin(S::Partial { detail: "`vm resize` rewrites vcpus/memory in the record and `vm start` rebuilds the vmm command line from it; no battery check yet" }),
        C::VmHotplug => S::NotImplemented,
