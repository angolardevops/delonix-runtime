...
pub fn libvirt_report(host: &LibvirtHost) -> ProviderReport {
    ...
    ProviderReport::build(
        ...
        C::VmDestroy => bin(S::Supported { evidence: "check:vm rm desfaz o domínio, a reserva DHCP e o overlay" }),
        C::VmRestart => bin(S::Supported { evidence: "check:vm restart: o domínio volta com outro id e a correr" }),
        ...
        C::VmExtraDisks => bin(S::Supported { evidence: "check:libvirt: extraDisks — o domínio vivo tem o 2.º disco" }),
        C::VmExtraNics => bin(S::Supported { evidence: "check:libvirt: extraNics — o domínio vivo tem 2 interfaces de rede" }),
        ...
        C::VmTpm => bin(S::Partial { detail: "`<tpm model='tpm-crb'>` emulator; measured 2026-09-24 on Ubuntu 24.04/libvirt 10.0: QEMU dies on `swtpm.sock: Permission denied` — the host's swtpm socket ownership, not probed" }),
        C::VmCpuModel => bin(S::Supported { evidence: "check:libvirt: cpuModel/cpuTopology — host-model e 1x2x1 no domínio" }),
        C::VmCpuPinning => sys(S::Supported { evidence: "check:libvirt: cpuAffinity — o vcpupin está no domínio vivo" }),
        ...
        C::VmStaticIp => sys(S::Supported { evidence: "check:ip: a reserva DHCP MAC→IP existe na rede libvirt" }),
        ...
        C::VmBackupDisk => bin(S::Supported { evidence: "check:backup create vm com a VM a correr (snapshot externo + block-commit)" }),
        C::VmBackupQuiesced => bin(S::Partial { detail: "`--quiesce` asks the guest agent to freeze; whether the guest has one is not probed" }),
        C::VmBackupRestore => bin(S::Supported { evidence: "check:backup restore vm repõe o overlay apagado" }),
        ...
        C::VmConsoleVnc => bin(S::Supported { evidence: "check:vm vnc devolve um endereço 127.0.0.1:59NN" }),
...
pub fn cloud_hypervisor_report(host: &CloudHypervisorHost) -> ProviderReport {
    ...
    ProviderReport::build(
        ...
        C::VmDestroy => bin(S::Supported { evidence: "check:CH: vm rm mata o VMM e apaga o overlay" }),
        C::VmRestart => bin(S::Supported { evidence: "check:CH: vm restart — o VMM volta com outro pid" }),
        C::VmPause => bin(S::Supported { evidence: "check:CH: vm pause — o VMM diz Paused" }),
        C::VmResume => bin(S::Supported { evidence: "check:CH: vm unpause — o VMM volta a Running" }),
        ...
        C::VmAntispoof => bin(S::Supported { evidence: "check:CH: antispoof — a regra do tap está no holder" }),
        ...
        C::VmBackupDisk => bin(S::Supported { evidence: "check:CH: backup create vm com a VM parada leva o overlay" }),
        C::VmBackupQuiesced => S::UnsupportedByProvider { reason: "no guest agent channel; only an offline copy is consistent" },
        C::VmBackupRestore => bin(S::Supported { evidence: "check:CH: backup restore vm repõe o overlay apagado e a VM arranca" }),
