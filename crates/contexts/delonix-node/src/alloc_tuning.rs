...
pub fn limit_malloc_arenas() {
    #[cfg(target_env = "gnu")]
    if let Some(cap) = arena_cap(std::env::var_os("MALLOC_ARENA_MAX").is_some()) {
        // SAFETY: `mallopt` takes two ints and touches no memory of ours.
        unsafe {
            libc::mallopt(libc::M_ARENA_MAX, cap);
        }
    }
}

/// The cap to apply, given whether the operator already set one.
///
/// An explicit `MALLOC_ARENA_MAX` in the environment is the operator speaking,
/// and glibc reads it before we get here. Overriding it would make an escape
/// hatch that does not escape — so `None`. Pure, so the guard is testable
/// without writing to the process environment (which this workspace's
/// `arch_fitness` counts as debt: tests run in parallel and share it).
fn arena_cap(operator_set_one: bool) -> Option<i32> {
    if operator_set_one {
        None
    } else {
        Some(2)
...
mod tests {
    ...
    #[test]
    fn an_explicit_operator_value_is_not_overridden() {
        // Cannot assert on glibc's internal state from here; what IS assertable
        // is the decision (no cap when the operator set one), and that the
        // entry points are safe to call.
        assert_eq!(super::arena_cap(true), None);
        assert_eq!(super::arena_cap(false), Some(2));
