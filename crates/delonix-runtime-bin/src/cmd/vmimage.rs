...
impl VmImageStore {
    ...
    pub fn base_cache_path(&self, ubuntu_release: &str) -> PathBuf {
        ...
        // ver AGENTS.md) elimina `/` de `ubuntu_release`, impedindo que
...
fn describe_one(store: &VmImageStore, img: &VmImage) {
    ...
    // qcow2) — numa imagem puxada ficam `None`. Ver o gap conhecido no AGENTS.md.
    d.field(
        "Ubuntu",
...
/// round-trip byte-idêntico; ver AGENTS.md, secção "Imagem VM dourada".
...
fn cmd_build(
    ...
    // conter metacaracteres de shell). Achado de auditoria, ver AGENTS.md.
    ...
    if offline {
        ...
        // DHCP que nunca chega em hosts onde o passt está partido (ver AGENTS.md).
