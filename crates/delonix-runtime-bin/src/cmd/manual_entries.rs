...
pub static ENTRIES: &[Entry] = &[
    ...
    Entry {
        ...
        examples: &[
            ...
            ("one public URL for a local port, with no account and no public IP on this host", "delonix net tunnel expose --provider pinggy --local-port 8080"),
        ],
    ...
    Entry {
        ...
        group: "Networking",
        examples: &[
            ("a local port on the public internet, with no account and no public IP on this host", "delonix net tunnel expose --provider pinggy --local-port 8080"),
    ...
    Entry {
        ...
        group: "Lifecycle",
        examples: &[
            ("pinggy needs no account and no extra binary — it is plain ssh, and the free URL is ephemeral", "delonix net tunnel expose --provider pinggy --local-port 8080"),
            ("a name of your own, instead of the `tunnel-<port>` default", "delonix net tunnel expose --name demo --provider pinggy --local-port 8080"),
            ("ngrok, with the authtoken read from a secret rather than typed into argv", "delonix net tunnel expose --provider ngrok --local-port 8080 --token-secret ngrok-token"),
            ("a cloudflare quick tunnel — random trycloudflare.com URL, no account, `cloudflared` must be on PATH", "delonix net tunnel expose --provider cloudflare --local-port 8080"),
        ],
