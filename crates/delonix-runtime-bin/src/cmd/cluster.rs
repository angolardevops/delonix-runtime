...
/// CONTENT of the script). Security-audit finding, see AGENTS.md.
    ...
    if spec.etcd.mode != "stacked" {
        return Err(Error::Invalid(format!(
            "etcd.mode '{}' não suportado nesta versão — só 'stacked' (etcd externo fica para \
             uma iteração seguinte, ver AGENTS.md)",
            spec.etcd.mode
        )));
    }
...
/// AGENTS.md), so matching by the image's OWN naming convention is the one
