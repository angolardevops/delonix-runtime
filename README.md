...
| `delonix-runtime` / `delonix-runtime-bin` | The container runtime itself (clone/namespaces/cgroups, create/stop/exec, status reconciliation) plus the `delonix` CLI binary — grouped commands (`container`/`image`/`build`/`vm`/`volumes`/`network`), Docker-equivalent, no API/orchestrator/tenant. |
