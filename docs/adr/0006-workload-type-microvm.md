...
- **Builds on:** ADR-0001 (`kind: Workload` lowering). **Related:** `AGENTS.md` "kind: Workload"
...
- The golden k8s image boots **libvirt-only** (see `AGENTS.md`); it is not a `microvm` workload.
...
`force_microvm_backend` + tests + `examples/workload.yaml` + `AGENTS.md`). This completes the
