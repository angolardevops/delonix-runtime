...
| [0044](0044-p4-provider-ports-vmspec-and-launcher.md) | P4 — the `VmSpec`/`Extensions` provider port, `VmProvider`, `StateRepository<T>`, and `delonix-launcher` | **Proposed 2026-09-18** — closes ADR-0040's seven `arch_fitness.py` P4 exceptions; 4 of 5 required spikes done — D6's `StateRepository<T>` for `Store<Container>` (`docs/discovery/56_...md`), D5's `LaunchSpec` transport (fd wins, `docs/discovery/57_...md`), D1/D8's Proxmox `network`/`namespace` live (`namespace` already refused, `network` confirmed ignored, `docs/discovery/58_...md`), D1-D4's substitution spike (CH+libvirt converge live on the same `VmSpec`, real code kept in `delonix-vm::provider_spike`, `docs/discovery/59_...md`); only the two-binary launcher/holder spike (P4d) left before Accepted |
