...
| [0044](0044-p4-provider-ports-vmspec-and-launcher.md) | P4 — the `VmSpec`/`Extensions` provider port, `VmProvider`, `StateRepository<T>`, and `delonix-launcher` | **Proposed 2026-09-18** — closes ADR-0040's seven `arch_fitness.py` P4 exceptions; 2 of 5 required spikes done — D6's `StateRepository<T>` for `Store<Container>` (`docs/discovery/56_...md`), D5's `LaunchSpec` transport (fd wins, `docs/discovery/57_...md`); 3 more required before Accepted |
