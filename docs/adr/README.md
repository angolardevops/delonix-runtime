...
| [0046](0046-vm-expose-ippool-hosts.md) | `VirtualMachine.spec.expose` (lowers to an `HTTPRoute`), `kind: IPPool`, and hosts injection in three targets | **Accepted 2026-09-20** — phase 1 (`expose` for Cloud Hypervisor VMs) and phase 1b (`hosts: host`, one delimited block in the operator's `/etc/hosts`) built and measured against the real proxy; `hosts: containers` withdrawn (see 0047); libvirt, `IPPool` and `hosts: guest` are later phases |
