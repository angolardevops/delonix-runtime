...
- **Status:** Proposed — the endpoint matrix exists and is reproducible (D1); the live
  conformance of slices 1–3 does not, and the status does not change before it does
- **Date:** 2026-09-23
...
2026-09-23 as above. The schema is not vendored (4 MB of JavaScript that changes per
...
|---|---|---|
| 0 | Versioned endpoint inventory (this ADR, `scripts/proxmox_api_inventory.py`) | Schema source, release, method/path denominator, three states with reasons, called routes read from the source; baseline reproduced by someone else with the docstring command — **done for 9.2.2** |
| 1 | Hardened transport and task handle | Response bound; secret provably absent from errors, `Debug` and `tracing` (a test that greps the rendered output for the token); typed status/errors mapped to `DX-` codes; UPID kept and terminal `exitstatus` checked on EVERY write (a test that every write site goes through `task`); failure injection: 401, malformed body, task `exitstatus` ≠ OK, timeout with the task still running |
