...
- **Related:** `CLAUDE.md` ("Por fazer, deliberadamente: `--format json`… merece desenho próprio"),
...
1. **Flag:** `-o, --output <table|json>`, default `table`. (`-o` is the kubectl/platform standard;
   supersedes the older `--format` wording in `CLAUDE.md` — Docker's `--format` means Go templates,
...
- **`--format '{{json .}}'` (Docker Go-template style).** Rejected: a template engine in Rust is a
  large surface for little gain; `-o json` covers the real need. (This is why `CLAUDE.md`'s
