...
- **Compatibilidade de ecossistema (limita o âmbito, não a segurança)** — multi-stage build e mutações da Docker Engine API (`docker compose up/down` via `create`+`start`) já FEITOS; BuildKit-lite (`--mount=secret`/`--platform`) parcial (v0.27.0); ainda falta GPU/CDI real. Não impede um beta honesto sobre o que cobre.
...
1. ✅ **FEITO (v0.26.0)**: `delonix docker-api` — leitura (2026-07-23) + mutações de ciclo de vida (`create`/`start`/`stop`/`kill`/`wait`/`restart`/`rename`/`remove`/`inspect`), validado contra um `docker` CLI real e o caminho `create`+`start` que o `docker compose up/down` usa. `exec`/attach interactivo continua fora de escopo.
