...
| **M13** | Maturidade e certificação | `IN_PROGRESS` | **`system features`**: 13 capacidades com nível E a evidência que o sustenta, com gates a recusar uma linha sem evidência, a impedir `certified` (a matriz que o define não existe) e a exigir ordem por maturidade. `cli-stability.md` + releases assinadas com SBOM e proveniência. **Sem** `release verify` nem `conformance report`. | `release verify`; gates de release por nível | 5 gates + 5 checks E2E | todos | médio | #126 | 3 de 5 |
