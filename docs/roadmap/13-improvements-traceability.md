...
| **M04** | Segurança verificável | `IN_PROGRESS` | Releases **assinadas** (minisign) + **proveniência SLSA** (`attest-build-provenance`, OIDC efémero) + **SBOM SPDX 2.3** de 380 pacotes, dentro do `SHA256SUMS` assinado. `cargo-deny` no CI. **Sem** fuzzing no CI, **sem** `kind: RuntimePolicy` (0 ocorrências). 3 auditorias ofensivas registadas. | `kind: RuntimePolicy`; job de fuzz; processo de advisory publicado | 4 gates E2E do SBOM; auditorias em `AGENTS.md` | — | médio | #125 | 2 de 4 |
...
| **M11** | Performance comprovada | `IN_PROGRESS` | `docs/comparacao-medida.md` compara as três na mesma máquina no mesmo dia — **da v0.53.0**. **`scripts/bench.sh` novo**: caracteriza a bancada, RECUSA correr acima de meio load, publica dispersão e densidade. A recontagem em 2026-08-25 foi **recusada pelo próprio harness** (load 55–72 em 32 threads) — está documentado, não convertido num número. | correr numa máquina ociosa; thresholds de regressão em CI | `bench.sh` + benches de `delonix-image` | M01, **máquina ociosa** | médio | #125 | 2 de 4 |
