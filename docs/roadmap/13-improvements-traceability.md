...
| **M05** | Desired State e GitOps | `IN_PROGRESS` | `stack` serve **11** verbos: `init apply destroy prune plan ls describe wait validate history rollback`. `plan` não muda estado e tem `--detailed-exitcode`; diff de 3 vias sem ficheiro de estado; **revisões persistidas** (ADR-0019). **`diff <kind> <name>` FEITO (#200, 2026-08-31)** — as três faces (manifesto, last-applied, observado) lado a lado para UM recurso nomeado, verbo de topo próprio, não o `plan` com outro nome. **Falta `drift`** (uma verificação de deriva sem alvo nomeado, contra o stack inteiro — hoje só o `plan --detailed-exitcode` cobre isso) **e `reconcile`** contínuo, que traz a pergunta do daemon. O `apply` continua fail-fast sem rollback, por desenho, e já não deixa órfãos invisíveis. | `drift` como verbo próprio; reconciler opcional com rate-limit | caos `stack_converge` + `stack_partial_apply`; **19 checks E2E** de `history`+`rollback`, um a apagar `stacks/` e outro a exigir o ciclo completo | M01 | médio | #120, #121, #122, #200 | 6 de 6 |
