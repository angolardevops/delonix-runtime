...
| **M04** | Segurança verificável | `IN_PROGRESS` | Releases assinadas (minisign) + proveniência SLSA + SBOM SPDX de 380 pacotes dentro da assinatura. `cargo-deny` no CI. **`policy.json` novo**: tecto do NÓ imposto no `cmd_run` — nega `--privileged`, `--net host`, `:latest` e registos fora da lista, fail-closed e com todas as razões de uma vez. **Job de fuzz no CI (2026-09-22)**: `fuzz/` (crate próprio, cargo-fuzz), dois alvos — `parse_dockerfile` (entrada não confiável de um Dockerfile/Delonixfile clonado) e `parse_reference` (a referência de imagem, já teve um bug real corrigido) —, 60s de smoke por PR. O `kind: RuntimePolicy` declarativo fica por fazer (hoje é ficheiro) — semântica de teardown de uma política de segurança (o que significa «destruir» um tecto do nó) é decisão do dono, não assumida aqui. | Kind declarativo; processo de advisory | 7 gates + 6 checks E2E + 2 alvos de fuzz | — | médio | #125, #126, #461 | 3 de 4 |
