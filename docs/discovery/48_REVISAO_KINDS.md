...
escreve à mão. E os dois **colidem**: o CLAUDE.md já documenta que um `to` que
...
ou VM, que é a promessa do Runtime Abstraction Layer que o CLAUDE.md fixa como
...
| **Proxmox** | 🔴 remoto | API REST de um cluster. O CLAUDE.md **já fixou** que inventário/scheduler multi-cluster é do `delonix-paas`. Um `ProxmoxBackend` de **nó único** cabe; o produto Proxmox não |
