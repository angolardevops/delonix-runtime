...
sessão, só por teste. É a mesma restrição já registada no `CLAUDE.md`.
...
- **O CRÍTICO da v0.37.0 (`proto: any` a descartar a porta) está fechado, agora provado ao
  vivo.** O `CLAUDE.md` dava-o como «provado por teste unitário, NÃO ao vivo» porque o
...
> (`CLAUDE.md`, secção do reverse-proxy).
...
2. O holder **não tem chain de `input`** (facto já registado no `CLAUDE.md`, ali como razão
