...
| `net netns up` (pin) | **rc=1** `timeout waiting for the netns holder` | rc=1 (ver abaixo) |
...
2. **Holder: NÃO como está implementado.** O pin não cria o namespace sozinho: arranca com
   `Command::new("unshare") --user --map-root-user --net --mount -- <exe> netns pin`
   (`delonix-net/src/infra.rs`, `start_pin`). Quem cria o user namespace é o
   **`/usr/bin/unshare`**, que cai no perfil restrito `unprivileged_userns` — o audit do
   kernel mostra `comm="unshare"` e um `mount` recusado. Um perfil com o nome do holder
   nunca se aplica. Dar `userns` ao `/usr/bin/unshare` abriria user namespaces a qualquer
   utilizador e anularia a restrição — não é opção.
...
- O holder com criação de namespaces dentro do processo **não foi escrito** — o GO do
  holder assenta na evidência do launcher (mesma operação, mesmo perfil), não num protótipo
  do holder.
