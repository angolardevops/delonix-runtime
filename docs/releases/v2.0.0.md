...
- **O regulador não vigia sozinho.** Não há daemon: é `--once` num temporizador
  ...
  seu próprio ADR.
- **O I/O continua sem regulação**, e não é uma escolha: o `io` não é delegável a
  um utilizador sem privilégios, portanto não há botão para rodar. O que existe é
  a VISIBILIDADE — o `io.pressure` de cada folha é contabilizado pelo kernel mesmo
  sem o controlador delegado, e dá para ver quem espera pelo disco em máquinas
  onde não dá para o limitar.
