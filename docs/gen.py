...
persistente — ao contrário de <code>rm</code>+<code>run</code>, nada do que o container escreveu se perde.</p>"""},
            ...
            "exec": {"examples": [("Shell interactiva — sem comando, tenta bash e cai para sh", "delonix container exec -it web")]},
...
próprio utilizador). Fora de escopo: <code>exec</code>/attach interactivo (HTTP hijacking);
<code>HostConfig.RestartPolicy</code> é servido (o arranque corre num processo novo, onde o supervisor é seguro);
...
sozinho onde encontrar um destes editores.""",
    ...
    "explain": {
        "title": "delonix explain",
        "tagline": "Referência de campos de um Kind, gerada do próprio código — `kubectl explain` style.",
        "intro": """Lê da MESMA fonte que o <code>manifest schema</code> (ADR-0007) — um schema
escrito à mão diverge do motor, e este repositório já pagou essa divergência três vezes — para
responder no terminal o que um editor apontado ao schema mostra em linha. O schema é
<strong>estável</strong> dentro do <code>0.x</code>: um campo nunca é removido nem muda de tipo ou
de significado — ver a
...
<code>scripts/install.sh</code> installs it wherever it finds one of these editors.""",
    ...
    "explain": {
        "tagline": "Field reference for a Kind, generated from the code — `kubectl explain` style.",
        "intro": """Reads from the SAME source as <code>manifest schema</code> (ADR-0007) — a
hand-written schema drifts from the engine, and this repository has paid for that drift three
times — so the terminal answers what an editor pointed at the schema shows inline. The schema is
<strong>stable</strong> within <code>0.x</code>: a field is never removed and never changes type or
meaning — see the
...
<tr><td><code>docker exec</code>/attach interactivo via a API</td>
<td>Docker ou Podman — deliberadamente fora de escopo (hijacking HTTP)</td></tr>
...
deliberadamente fora de escopo (hijacking HTTP).</li>
...
<tr><td>Interactive <code>docker exec</code>/attach via the API</td>
<td>Docker or Podman — deliberately out of scope (HTTP hijacking)</td></tr>
