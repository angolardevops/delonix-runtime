...
| **Container Engine** (OCI/CRI/rootless/ns/cgroup v2/seccomp/caps) | ✅ | Real and mature (`delonix-linux` + `delonix-cri`). AppArmor/SELinux are abstracted, not enforced — closing that is a scoped, in-bounds task. **OCI runtime hooks** (`prestart`/`poststart`/`poststop`/…) are the one named conformance gap — see [ADR-0033](../adr/0033-oci-runtime-hooks.md): no concrete consumer, same security class dynamic plugin loading is gated behind below. |
