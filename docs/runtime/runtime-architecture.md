...
| **Container Engine** (OCI/CRI/rootless/ns/cgroup v2/seccomp/caps) | ✅ | Real and mature (`delonix-runtime` + `delonix-cri`). AppArmor/SELinux are abstracted, not enforced — closing that is a scoped, in-bounds task. **OCI runtime hooks** (`prestart`/`poststart`/`poststop`/…) are the one named conformance gap — see [ADR-0033](../adr/0033-oci-runtime-hooks.md): no concrete consumer, same security class dynamic plugin loading is gated behind below. |
...
| **Storage Engine** (local/Ceph/ZFS/LVM/NFS/SMB/Object/CSI; snapshot/clone/quota/encrypt/replicate) | 🟡 / ⛔ | Local volumes + network storage (NFS/CIFS/WebDAV) exist. **Ceph/ZFS/LVM/Object/CSI = ⛔ absent.** A `StorageDriver` trait is a reasonable Phase-2 target; CSI specifically edges toward orchestrator concerns — see [ADR-0034](../adr/0034-csi-daemon-conflict.md): a CSI Node plugin needs a persistent daemon, which conflicts with the daemonless guardrail, and `kind: Volume`'s NFS/CIFS/TrueNAS support already covers the practical need without one. |
| **Image Engine** | ✅ | `delonix-oci` — pull/build/registry/CNB/CAS/**signature verification**. |
...
| **Recovery Engine** (checkpoint/restore/DR/cyber-recovery) | ⛔ / 🚧 | **Verified stub** (2026-07-30): `checkpoint_container` = `Unimplemented`, zero CRIU in the repo — no foundation today. Only `set_frozen` (cgroup freeze) exists, which is not checkpoint. Container checkpoint/restore *could* fit but needs a **rootless-CRIU GO/NO-GO spike** first — see [ADR-0004](../adr/0004-container-checkpoint-restore.md). **Fleet DR / cyber-recovery orchestration is 🚧** (control-plane/PaaS). |
