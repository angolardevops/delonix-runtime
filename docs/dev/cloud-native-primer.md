...
- VMs on the rootless SDN get a DHCP lease derived from their MAC: `dhcp_lease_ip` in
  `crates/adapters/delonix-sdn/src/infra.rs`.
- Practical setup and known host pitfalls: [MicroVM setup](microvm-setup.md).
...
[Rust primer §3.8](rust-primer.md#38-concurrency-and-shared-state)), a supervisor process
