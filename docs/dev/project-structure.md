...
| `crates/foundation/` | Pure foundation: shared types and errors (`delonix-runtime-core`), the data-only model — records, errors, generated names, exit-code and `DX_*` classes (`delonix-model`), and zero-dependency network rules (`delonix-net-rules`). Depends only on the foundation. | Changes that add a shared type or pure rule. | [The crates](crates.md) |
| `crates/contexts/` | Bounded contexts with the use cases: Compute (`delonix-compute`), Stack — Kinds, reconciler, revisions (`delonix-stack`) — and the node's security decisions (`delonix-security-runtime`). | Features that change what the engine decides. | [The crates](crates.md) |
