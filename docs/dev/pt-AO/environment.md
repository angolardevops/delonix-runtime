...
Ver [Clonar, compilar e testar](build-and-test.md) para saber como cada uma é corrida.
...
| **KVM** (`/dev/kvm`) | só para microVMs — ver [Construir microVMs](microvm-setup.md) | `test -w /dev/kvm` |
...
Cloud Hypervisor com o seu firmware — ver [Construir microVMs](microvm-setup.md). Para construir imagens de VM,
...
Compila o binário (ver [Clonar, compilar e testar](build-and-test.md)) e pergunta-lhe. Os comandos abaixo são só de
...
do Delonix em uso, isola primeiro o estado (ver [Isolar o estado do motor](build-and-test.md#isolating-the-engines-state)).
