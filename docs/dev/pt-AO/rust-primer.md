...
de língua (LANG-01) e o seu gate são tratados em [Fluxo de contribuição](contributing-workflow.md).
...
(`scripts/contract_gate.py`, `buf`); ver [Compilar e testar](build-and-test.md).
...
- **Os testes não podem tocar no estado real do host.** Obtém um directório temporário e passa-o
  ...
  descrita em [Compilar e testar](build-and-test.md).
...
A lista completa e a forma de correr cada um localmente estão em [Compilar e testar](build-and-test.md).
