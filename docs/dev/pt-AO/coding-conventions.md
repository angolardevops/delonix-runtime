...
adivinhar as regras nem de inventar as tuas. Depois dela consegues aplicar a lista de verificação
do fim ao teu próprio diff antes de um revisor o fazer. Cada regra abaixo leva uma etiqueta e uma fonte:

- **Imposto (gate)**: um job de CI falha se a quebrares. O gate é nomeado, para o poderes correr
  localmente (vê [Construir e testar](build-and-test.md#the-gates-ci-runs)).
...
Páginas relacionadas: as camadas e as razões por trás delas em [Arquitectura](architecture.md),
o que cada crate contém em [Crates](crates.md), os idiomas de Rust em
[Introdução ao Rust](rust-primer.md), e o fluxo de trabalho (worktree, versão, ADRs, PRs) em
...
| **`scripts/arch_fitness.py`** | Direcção das camadas, directório do crate = camada, versões só na raiz, nomes de consumidores, e os ratchets de dívida listados em [Arquitectura](architecture.md#layers-and-the-allowed-direction) (vê [§4](#4-structure-where-code-goes) e [§5](#5-internal-api-and-boundaries)). | **Imposto (gate)**: job de CI `arch`, linha de base em `scripts/arch_baseline.json`. |
...
- **Convenção de nomes alvo para crates novos ou reestruturados.** **Proposto (ADR, ainda não
  ...
  esse crate, `delonix-runtime-core`, foi removido na #406). Alguns crates ainda têm nomes
  antigos: `delonix-proxmox`, `delonix-truenas`, `delonix-security-runtime`. O ADR-0040 renomeia
  cada um na fase que o reestrutura, «never twice». **Não renomeies um crate fora da sua fase.**
...
- **Um comando alcançável por vários caminhos tem de estar ligado em todos eles** (por exemplo
  ...
  [Fluxo de contribuição](contributing-workflow.md#adding-or-changing-a-cli-command).
- **As mudanças de folhas actualizam a linha de base da CLI** (`scripts/cli-tree.sh --update`) no
  mesmo commit. **Imposto (gate)**: vê [Fluxo de contribuição](contributing-workflow.md#adding-or-changing-a-cli-command).
...
- **Os registos internos** (o JSON debaixo do state root) mantêm os nomes de campo `snake_case` do
  Rust. Vê `crates/contexts/delonix-compute/src/record.rs` (`net_mode`, `namespace`).
  **Convenção (observada)**.
...
tabela de camadas gerada e as excepções actuais estão em [Arquitectura](architecture.md#layers-and-the-allowed-direction).
...
Esta tabela usa os crates tal como existem hoje. Consulta [Os crates](crates.md) para ver o conteúdo de
...
| Um tipo de registo de carga persistido (`Container`, `Vm`, `Mount`, …) | `delonix-compute` (context): `record.rs` | ADR-0040 D2.2; #406 |
| Um registo de dados simples sem mecanismo (`Status`, `ContainerFw`/`FwRule`, `typestate`) | `delonix-model` (foundation): `records.rs`, `typestate.rs` | ADR-0040 P3 (#405) |
| Uma pergunta feita ao host ou a um processo (`now_unix`, vivacidade de pid, user namespace, geração de ids), o registo de eventos, a regra de despacho do servidor, `SO_PEERCRED` | `delonix-node` (context) | ADR-0040 D2.2; #406 |
...
- **Um adapter ou provider define o seu próprio `Error` e converte-o na classe partilhada**
  ...
  **Imposto (gate)**: o ratchet `shared_error_imports` no `arch_fitness.py` (`SHARED_ERROR`,
  ...
  `use delonix_model::{…Error/Result…}` que tornam o tipo
...
- **Nenhum `clone()`/`fork()` cru num processo multi-thread** (os servidores tokio, o shim da API
  ...
  [Introdução ao Rust para esta base de código](rust-primer.md#why-forkclone-in-a-multi-threaded-process-is-dangerous).
...
- **Antes de sinalizar um pid lido de um ficheiro, confirma que ainda é o mesmo processo.** Usa
  `delonix_node::safe_to_signal(pid, starttime)`, que compara o instante de arranque para um
...
- **Os campos novos em registos persistidos levam `#[serde(default)]`** (ou `default = "fn"`), para
  ...
  `Vm.namespace`, `VmImage.cloud_init`). **Convenção (observada)**: `delonix-compute/src/record.rs`,
...
- **Onde vão.** Os testes unitários vão num `#[cfg(test)] mod tests` no fim do ficheiro. Os testes
  ...
  providers vivos são opt-in. **Convenção (observada)**; detalhes em [03 § 3.9](rust-primer.md#39-tests).
...
- **Quando moves uma fronteira, actualiza os seus registos na mesma mudança**:
  - um ADR **antes** do código, para um provider, porta ou daemon novo, uma dependência externa num
    ...
    [Fluxo de contribuição](contributing-workflow.md#when-to-write-an-adr));
  ...
  - os artefactos gerados: `docs/schema/v1/delonix.json`, `docs/api/openapi.yaml`, a linha de base
    ...
    [Publicar a documentação](publishing-docs.md)).
...
- **O tipo `Result` das portas de compute de hoje.** As portas em
  `delonix-compute/src/ports.rs` devolvem `delonix_model::Result`, por isso os adapters que
...
**Seguinte:** [Acrescentar um Kind](adding-a-kind.md) — a tabela, o schema e a ligação ao reconciliador de que um Kind declarativo novo precisa, trabalhados através de um real.
