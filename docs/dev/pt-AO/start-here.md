...
([Clonar, compilar e testar](build-and-test.md#clone)).
...
mensagem sobre o `protoc`, instala-o ([Preparar o teu ambiente](environment.md#protoc-required-to-build)).
...
release instalada e normalmente é mais antigo ([Preparar o teu ambiente](environment.md#a-stale-delonix-on-your-path)).
...
(4 = não existe — ver
[Introdução ao Rust para esta base de código](rust-primer.md#32-errors-one-error-and-exit-codes-derived-from-its-type)). Se
...
explicada em
[Preparar o teu ambiente](environment.md#cgroup-delegation-some-limits-are-refused-others-are-not-enforced).
...
- Para tudo o que não seja trivial — um comando novo, um Kind de manifesto novo, uma mudança na
  ...
  abordagem ([`CONTRIBUTING.md`](../../../CONTRIBUTING.md), [Fluxo de contribuição](contributing-workflow.md)).
...
em `/tmp` ([Um worktree por tarefa](contributing-workflow.md#one-worktree-per-task)):
...
tentadas, medidas e mudadas ([Parte da tag mais recente](contributing-workflow.md#start-from-the-latest-tag-not-from-memory)).
...
- Uma função pura nova (parser, validador, construtor de argumentos, plano) leva um teste unitário
  ...
  passa-lhe um directório temporário — ver [Testes](rust-primer.md#39-tests).
...
- Uma mudança em namespaces, cgroups, no holder de rede ou no arranque de VMs precisa também de uma
  ...
  ([Correr os testes](build-and-test.md#run-the-tests)).

### 5. Corre os gates locais

Cada job de CI tem um comando local, listado em
[Os gates que a CI corre](build-and-test.md#the-gates-ci-runs). No mínimo, antes de pedir
revisão:
...
([Commits e pull requests](contributing-workflow.md#commits-and-pull-requests)).

### 7. O que os revisores verificam

Os code owners em [`.github/CODEOWNERS`](../../../.github/CODEOWNERS) revêem todas as mudanças. A
checklist de revisão está em [Convenções de código](coding-conventions.md); os padrões cloud
native contra os quais uma mudança é medida estão em
[Padrões cloud native](cloud-native-standards.md). Lê as duas antes de abrires o PR, não
...
| **Um crate novo** | Uma entrada em `LAYERS` em `scripts/arch_fitness.py`, um directório em `crates/<layer>/`, e o seu caminho no `[workspace.dependencies]` do `Cargo.toml` raiz — tudo no mesmo commit | [Camadas](architecture.md#layers-and-the-allowed-direction) | `scripts/arch_fitness.py` (directório = camada, versões só na raiz) |
| **Uma decisão que muda uma fronteira** | `docs/adr/NNNN-title.md`, antes do código | [Quando escrever um ADR](contributing-workflow.md#when-to-write-an-adr) | [`docs/adr/README.md`](../../adr/README.md); os ADRs aceites são sucedidos, nunca reescritos |
...
| **Daemonless.** Nenhum processo residente por omissão; um novo precisa de um ADR com a evidência do que o systemd não conseguiu fazer. | [`AGENTS.md`](../../../AGENTS.md) (a mesma secção); [Regras de arquitectura](contributing-workflow.md#architecture-rules-the-gates-enforce) |
| **Rootless-first.** O caminho normal corre sem privilégios; o privilégio é um opt-in explícito e anunciado. Uma fronteira de privilégio nova precisa de um spike GO/NO-GO e de um ADR. | [`AGENTS.md`](../../../AGENTS.md); [Quando escrever um ADR](contributing-workflow.md#when-to-write-an-adr) |
...
| **LANG-01: o código é em inglês.** Identificadores, comentários e mensagens em inglês; português só através do `pt.po`. | [Língua](contributing-workflow.md#language-english-in-the-code-lang-01); `scripts/lang_ratchet.py` |
| **Alinhamento da versão.** Não mudes a `version` do `Cargo.toml` raiz num PR de feature; o teu branch tem de conter a tag mais recente. | [Alinhamento da versão](contributing-workflow.md#version-alignment); `scripts/version_gate.py` |
| **Um worktree por tarefa**, fora de `/tmp`, ficheiros adicionados pelo nome, worktree e branch removidos no fim. | [Um worktree por tarefa](contributing-workflow.md#one-worktree-per-task) |
| **Nunca corras o motor, a bateria E2E ou o arnês de caos contra estado real.** Exporta tanto `DELONIX_ROOT` como `DELONIX_NET_RUNTIME_DIR`; não definas `E2E_SHARED_STATE=1` a não ser que estejas a diagnosticar o teu próprio host. | [Isolar o estado do motor](build-and-test.md#isolating-the-engines-state), [E2E](build-and-test.md#end-to-end-battery-scriptse2esh), [caos](build-and-test.md#chaos-harness-scriptschaossh) |
| **As mudanças sensíveis para a segurança são assinaladas**, e as vulnerabilidades são reportadas em privado, nunca numa issue ou PR público. | [`SECURITY.md`](../../../SECURITY.md); [Mudanças sensíveis para a segurança](contributing-workflow.md#security-sensitive-changes) |

## Quando estás preso

Procura por esta ordem — cada passo é mais barato do que o seguinte:

1. **Este manual.** O [README](README.md) tem a lista de páginas; o [glossário](glossary.md)
...
- [ ] Li o que é o Delonix e os quatro princípios ([Arquitectura](architecture.md#engine-identity-and-boundaries)).
- [ ] O meu host cumpre a [Preparar o teu ambiente](environment.md), e li as armadilhas conhecidas do host.
...
- [ ] Descobri onde vai a mudança e li a secção desse crate na [Os crates](crates.md).
- [ ] Escrevi um teste que falha sem a minha mudança.
- [ ] Os gates locais da [Clonar, compilar e testar](build-and-test.md#the-gates-ci-runs) passam.
