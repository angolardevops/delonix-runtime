...
# 13. Padrões cloud native, camada a camada
...
Lê primeiro [4. Introdução ao cloud native](04-cloud-native-primer.md) se namespaces, cgroups,
...
- **Artefactos OCI** (blob único, config vazia `application/vnd.oci.empty.v1+json`, o padrão que o
  ...
  desta forma (vê [9. Construir microVMs](09-microvm-setup.md)). As annotations só são lidas
...
- **SDN nativa** (a omissão para containers): o netns holder rootless, bridge, IPAM, firewall
  nftables — `crates/adapters/delonix-sdn` (vê [4.5](04-cloud-native-primer.md) e
  [5. Arquitectura](05-architecture.md)).
...
`contract_gate.py`), não por uma suite externa. Vê [5. Arquitectura](05-architecture.md) e
...
- **Backends atrás de uma porta**: `trait VmBackend` e `register_backend` em `src/lib.rs`. O Cloud
  ...
  remoto (`crates/providers/delonix-proxmox`). Vê [9. Construir microVMs](09-microvm-setup.md).
...
[1. Ambiente](01-environment.md#cgroup-delegation-some-limits-are-refused-others-are-not-enforced) e o [ADR-0015](../../adr/0015-intermediate-cgroup-level.md).
