...
See [Clone, build and test](build-and-test.md) for how each one is run.
...
| **KVM** (`/dev/kvm`) | only for microVMs — see [Building microVMs](microvm-setup.md) | `test -w /dev/kvm` |
...
Cloud Hypervisor with its firmware — see [Building microVMs](microvm-setup.md). For building VM images,
...
Build the binary (see [Clone, build and test](build-and-test.md)) and ask it. The commands below are
...
isolate the state first (see [Isolating the engine's state](build-and-test.md#isolating-the-engines-state)).
