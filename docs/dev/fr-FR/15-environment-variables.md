...
# 15. Variables d’environnement (`DELONIX_*`)
...
script et dans [02](02-build-and-test.md), pas ici.
...
| Backend de VM | `--backend` (ou le `HYPERVISOR` de l’image) > `DELONIX_VM_BACKEND` > `vm default-backend --set` > détection automatique | `crates/adapters/delonix-vm/src/lib.rs:standing_backend_choice` |
