...
- **Artefacts OCI** (blob unique, config vide `application/vnd.oci.empty.v1+json`, le modèle
  ...
  publiées ainsi (voir [Construire des microVMs](microvm-setup.md)). Les annotations ne sont lues
...
- **SDN native** (la valeur par défaut pour les containers) : le netns holder rootless, le bridge,
  l'IPAM, le pare-feu nftables — `crates/adapters/delonix-sdn` (voir [4.5](cloud-native-primer.md)
  et [Architecture](architecture.md)).
