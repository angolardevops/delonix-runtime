...
# 3. Initiation à Rust pour cette base de code
...
2. **Le répertoire est la couche.** Les crates vivent dans `crates/foundation/`, `crates/contexts/`,
   ...
   [5. Architecture](05-architecture.md) pour les règles des couches.
...
[2. Compiler et tester](02-build-and-test.md) pour le gate de version.
...
| [`libc`](https://docs.rs/libc) | Appels bruts que `nix` n’enveloppe pas, ou lorsque la structure exacte compte | `libc::getsockopt(.., SO_PEERCRED, ..)` dans `crates/foundation/delonix-runtime-core/src/peer_cred.rs` (`peer_uid`) ; `libc::flock` dans `store.rs` |
...
de langue (LANG-01) et son gate sont traités dans [10. Flux de contribution](10-contributing-workflow.md).
...
(`scripts/contract_gate.py`, `buf`) ; voir [2. Compiler et tester](02-build-and-test.md).
...
`JsonStore<T>::update` dans le même fichier est la version générique pour les autres types d’enregistrement. Règles
qui en découlent :
...
- **Les tests ne doivent pas toucher l’état réel de l’hôte.** Prenez un répertoire temporaire et passez-le
  ...
  réelle/E2E décrite dans [2. Compiler et tester](02-build-and-test.md).
...
La liste complète et la façon d’exécuter chacun localement se trouvent dans [2. Compiler et tester](02-build-and-test.md).
