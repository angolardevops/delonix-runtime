...
suppose que vous avez déjà lu [01 — Préparer votre environnement](01-environment.md) et que vous
savez construire l'arborescence ([02](02-build-and-test.md)).
...
| `virt-customize`, `virt-sparsify`, `virt-copy-out` | `libguestfs-tools` / `guestfs-tools` | `image vm build` uniquement |
...
[02](02-build-and-test.md)). Une VM Cloud Hypervisor refuse aussi d'emblée lorsque
...
`delonix image vm build -t <tag>` sans `VMfile` dans le contexte exécute la recette intégrée :
...
recette est un `VMfile` — voir [08 — Delonixfile et VMfile](08-delonixfile-and-vmfile.md). Les
...
- Implémentez `VmBackend` dans **son propre crate** s'il parle à une API distante (le crate du moteur
  ...
  `scripts/arch_fitness.py` (voir [05 — Architecture](05-architecture.md)).
...
- **Tests unitaires purs** — les constructeurs d'argv et de XML sont des fonctions pures, ils se
  ...
  [02](02-build-and-test.md) pour `protoc` et le répertoire cible).
