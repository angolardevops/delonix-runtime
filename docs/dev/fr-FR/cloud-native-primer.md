...
AppArmor — voir [Environnement](environment.md) pour les conséquences pratiques.
...
- Les VM sur le SDN rootless reçoivent un bail DHCP dérivé de leur MAC : `dhcp_lease_ip` dans
  `crates/adapters/delonix-sdn/src/infra.rs`.
- Configuration pratique et pièges connus de l’hôte : [Configuration des microVM](microvm-setup.md).
...
[Initiation à Rust §3.8](rust-primer.md#38-concurrency-and-shared-state)), un processus superviseur
...
bonnes et mauvaises — sont discutées dans [Architecture](architecture.md) et
