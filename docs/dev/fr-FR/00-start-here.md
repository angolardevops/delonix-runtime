...
une étape à la fois. Chaque étape dit quoi faire, ce que vous devriez voir, et quelle page explique les
détails. Chaque règle de cette page renvoie à l’endroit où elle est écrite — si une règle n’a pas de lien, ce
n’est pas une règle.

Si un mot vous est inconnu, cherchez-le dans le [glossaire](14-glossary.md).
...
code. Lisez [05 — Architecture](05-architecture.md#engine-identity-and-boundaries) pour le tableau
complet ; pour l’instant, ces quatre phrases suffisent.
...
des bugs du moteur, se trouvent dans [01 — Préparer votre environnement](01-environment.md). Lisez au moins sa section
[Pièges connus de l’hôte](01-environment.md#known-host-traps) avant l’étape 5 ci-dessous.
...
message au sujet de `protoc`, installez-le ([01](01-environment.md#protoc-required-to-build)).
...
release installée et est généralement plus ancien ([01](01-environment.md#a-stale-delonix-on-your-path)).
...
[03](03-rust-primer.md#32-errors-one-error-and-exit-codes-derived-from-its-type)). Si
...
- Pour tout ce qui n’est pas trivial — une nouvelle commande, un nouveau Kind de manifeste, une modification de la configuration des namespaces ou des
  ...
  ([`CONTRIBUTING.md`](../../../CONTRIBUTING.md), [10](10-contributing-workflow.md)).
...
([10 — Un worktree par tâche](10-contributing-workflow.md#one-worktree-per-task)) :
...
ont été essayées, mesurées et modifiées ([10 — Partez du dernier tag](10-contributing-workflow.md#start-from-the-latest-tag-not-from-memory)).
...
section de [06 — Les crates](06-crates.md) consacrée à ce crate.
...
- Une nouvelle fonction pure (parseur, validateur, constructeur d’arguments, plan) reçoit un test unitaire dans le même fichier,
  ...
  répertoire temporaire — voir [03 — Tests](03-rust-primer.md#39-tests).
...
- Une modification des namespaces, des cgroups, du holder réseau ou du démarrage des VM nécessite aussi une exécution **réelle** avec
  ...
  ([02 — Exécuter les tests](02-build-and-test.md#run-the-tests)).

### 5. Exécutez les gates locaux

Chaque job de CI a une commande locale, listée dans
[02 — Les gates exécutés par la CI](02-build-and-test.md#the-gates-ci-runs). Au minimum, avant de demander une
revue :
...
contrat si vous avez touché `proto/`, le générateur de documentation si le texte d’aide a changé) — le tableau de 02 indique lesquels.
...
([10 — Commits et pull requests](10-contributing-workflow.md#commits-and-pull-requests)).
...
liste de contrôle de la revue se trouve dans [12 — Conventions de code](12-coding-conventions.md) ; les standards cloud native à l’aune
desquels une modification est mesurée se trouvent dans [13 — Standards cloud native](13-cloud-native-standards.md). Lisez
...
|---|---|---|---|
| **Nouvelle option ou sous-commande de la CLI** | `bins/delonix-runtime-bin/src/cmd/<group>.rs` (un module par groupe) ; chaînes via `cmd/po.rs` avec le portugais dans `data/pt.po` ; texte du manuel dans `cmd/manual_entries.rs` ; liste des feuilles dans `scripts/cli_baseline.tsv` (`scripts/cli-tree.sh --update`). La validation pure d’une exécution appartient à `crates/contexts/delonix-compute/src/preflight.rs` | [10 — Ajouter ou modifier une commande de la CLI](10-contributing-workflow.md#adding-or-changing-a-cli-command), [03 — La CLI](03-rust-primer.md#36-the-cli-clap-derive-and-translated-output) | LANG-01 (`scripts/lang_ratchet.py`) ; gate de surface de la CLI (`scripts/cli-tree.sh --gate`, `scripts/docs_cli_gate.py`) ; câbler chaque point d’entrée (`CONTRIBUTING.md`) |
| **Nouveau Kind, ou un champ d’un Kind** | Les faits du Kind : `FACTS` dans `crates/contexts/delonix-stack/src/kinds.rs`. Son type de spec et son apply : `bins/delonix-runtime-bin/src/cmd/<kind>.rs`. Champs modifiables à chaud : `hot_fields` dans `crates/contexts/delonix-stack/src/reconcile.rs`. Le schéma : `TYPED_KINDS` dans `cmd/schema.rs`, et le `docs/schema/v1/delonix.json` publié (`delonix manifest schema`) | [04 — Réconciliation déclarative](04-cloud-native-primer.md#48-declarative-reconciliation), [06 — `delonix-stack`](06-crates.md#delonix-stack) | Ouvrir d’abord une issue (`CONTRIBUTING.md`) ; le schéma est généré à partir du code ([ADR-0007](../../adr/0007-generated-manifest-schema.md)) ; les tests de `kinds.rs` et `schema.rs` échouent lorsqu’une table est oubliée |
| **Comportement réseau** | Règles pures sans I/O : `crates/foundation/delonix-net-rules/src/lib.rs`. Dataplane (holder, socket de contrôle, nftables, IPAM, CNI) : `crates/adapters/delonix-sdn/src/` (`infra.rs`, `ipam.rs`, `cni.rs`). L’étape réseau de `container run` : `crates/contexts/delonix-compute/src/network.rs`. CLI : `cmd/network.rs`, `cmd/net.rs`, `cmd/firewall.rs` | [04 — Réseau des containers](04-cloud-native-primer.md#45-container-networking), [06 — `delonix-sdn`](06-crates.md#delonix-sdn) | Rootless-first et pas d’échec silencieux ([10 — Règles d’architecture](10-contributing-workflow.md#architecture-rules-the-gates-enforce)) ; signaler la frontière de privilège dans la PR (`SECURITY.md`) |
| **Images, registre, build** | `crates/adapters/delonix-oci/src/` (`registry.rs`, `build.rs`, `cas.rs`, `overlay.rs`) ; CLI dans `cmd/build.rs`, `cmd/image.rs` | [08 — Delonixfile et VMfile](08-delonixfile-and-vmfile.md), [06 — `delonix-oci`](06-crates.md#delonix-oci) | Les téléchargements sont vérifiés par digest (`SECURITY.md`, périmètre supply chain) |
| **État persisté : un champ d’enregistrement, un store, le verrouillage de fichiers, les secrets au repos** | Types d’enregistrement (`Container`, `Vm`) : `crates/foundation/delonix-runtime-core/src/lib.rs`. Comment ils sont stockés et verrouillés (`Store`, `JsonStore`, `write_atomic*`, `SecretStore`, `CredVault`) : `crates/adapters/delonix-state/src/` (`store.rs`, `secret.rs`, `cred_vault.rs`) | [06 — `delonix-state`](06-crates.md#delonix-state), [05 — L’état sur disque](05-architecture.md#state-on-disk), [03 — Concurrence](03-rust-primer.md#38-concurrency-and-shared-state) | Les nouveaux champs d’enregistrement prennent `#[serde(default)]` ; la lecture-modification-écriture passe par `update` ([12 — État et concurrence](12-coding-conventions.md#8-state-and-concurrency)) |
| **Comportement des VM sur ce nœud** | `crates/adapters/delonix-vm/src/lib.rs` (le trait `VmBackend` et le registre de backends), `cloudinit.rs` ; CLI dans `cmd/vm.rs`, `cmd/vmimage.rs`, `cmd/vmfile.rs` | [09 — Construire des microVMs](09-microvm-setup.md), [03 — Les traits comme ports](03-rust-primer.md#33-traits-as-ports-vmbackend-and-the-backend-registry) | [ADR-0008](../../adr/0008-proxmox-vm-backend.md) (les backends sont enregistrables) |
| **Un nouveau backend de VM ou provider de stockage derrière une API distante** | Un nouveau crate sous `crates/providers/`, qui implémente un port ; enregistré à la racine de composition (`cmd/vmbackends.rs`) | [06 — Providers](06-crates.md#providers), [05 — Les couches](05-architecture.md#layers-and-the-allowed-direction) | ADR d’abord ([10 — Quand écrire un ADR](10-contributing-workflow.md#when-to-write-an-adr)) ; [ADR-0040](../../adr/0040-engine-restructuring-layers-ports-node-contract.md) |
| **CRI (ce à quoi parle le kubelet)** | `crates/interfaces/delonix-cri/src/` (`runtime_svc.rs`, `runtime_svc/lifecycle.rs`, `streaming.rs`) | [04 — Kubernetes](04-cloud-native-primer.md#46-kubernetes-cri-kubelet-kubeadm-and-kind), [06 — `delonix-cri`](06-crates.md#delonix-cri) | [ADR-0038](../../adr/0038-cri-follows-kubelet-resource-model.md) |
| **Un nouveau crate** | Une entrée dans `LAYERS` dans `scripts/arch_fitness.py`, un répertoire sous `crates/<layer>/`, et son chemin dans `[workspace.dependencies]` du `Cargo.toml` racine — le tout dans le même commit | [05 — Les couches](05-architecture.md#layers-and-the-allowed-direction) | `scripts/arch_fitness.py` (répertoire = couche, versions uniquement à la racine) |
| **Une décision qui déplace une frontière** | `docs/adr/NNNN-title.md`, avant le code | [10 — Quand écrire un ADR](10-contributing-workflow.md#when-to-write-an-adr) | [`docs/adr/README.md`](../../adr/README.md) ; les ADR acceptés sont remplacés par un successeur, jamais réécrits |
...
| **Daemonless.** Aucun processus résident par défaut ; un nouveau exige un ADR avec la preuve de ce que systemd ne pouvait pas faire. | [`AGENTS.md`](../../../AGENTS.md) (même section) ; [10 — Règles d’architecture](10-contributing-workflow.md#architecture-rules-the-gates-enforce) |
| **Rootless-first.** Le chemin normal s’exécute sans privilège ; le privilège est un opt-in explicite et annoncé. Une nouvelle frontière de privilège exige un spike GO/NO-GO et un ADR. | [`AGENTS.md`](../../../AGENTS.md) ; [10 — Quand écrire un ADR](10-contributing-workflow.md#when-to-write-an-adr) |
...
| **LANG-01 : le code est en anglais.** Identifiants, commentaires et messages en anglais ; le portugais uniquement via `pt.po`. | [10 — Langue](10-contributing-workflow.md#language-english-in-the-code-lang-01) ; `scripts/lang_ratchet.py` |
| **Alignement de version.** Ne modifiez pas `version` dans le `Cargo.toml` racine dans une PR de fonctionnalité ; votre branch doit contenir le tag le plus récent. | [10 — Alignement de version](10-contributing-workflow.md#version-alignment) ; `scripts/version_gate.py` |
| **Un worktree par tâche**, en dehors de `/tmp`, ajoutez les fichiers par nom, supprimez le worktree et la branch à la fin. | [10 — Un worktree par tâche](10-contributing-workflow.md#one-worktree-per-task) |
| **N’exécutez jamais le moteur, la batterie E2E ou le harnais de chaos contre l’état réel.** Exportez à la fois `DELONIX_ROOT` et `DELONIX_NET_RUNTIME_DIR` ; ne définissez pas `E2E_SHARED_STATE=1` sauf si vous diagnostiquez votre propre hôte. | [02 — Isoler l’état du moteur](02-build-and-test.md#isolating-the-engines-state), [E2E](02-build-and-test.md#end-to-end-battery-scriptse2esh), [chaos](02-build-and-test.md#chaos-harness-scriptschaossh) |
| **Les modifications sensibles pour la sécurité sont signalées**, et les vulnérabilités sont signalées en privé, jamais dans une issue ou une PR publique. | [`SECURITY.md`](../../../SECURITY.md) ; [10 — Modifications sensibles pour la sécurité](10-contributing-workflow.md#security-sensitive-changes) |

## Quand vous êtes bloqué

Cherchez dans cet ordre — chaque étape coûte moins cher que la suivante :

1. **Ce manuel.** Le [README](README.md) contient la liste des pages ; le [glossaire](14-glossary.md)
...
- [ ] J’ai lu ce qu’est Delonix et les quatre principes ([05](05-architecture.md#engine-identity-and-boundaries)).
- [ ] Mon hôte satisfait [01](01-environment.md), et j’ai lu les pièges connus de l’hôte.
...
- [ ] J’ai trouvé où va la modification et lu la section de ce crate dans [06](06-crates.md).
- [ ] J’ai écrit un test qui échoue sans ma modification.
- [ ] Les gates locaux de [02](02-build-and-test.md#the-gates-ci-runs) passent.
- [ ] J’ai lu [12](12-coding-conventions.md) et [13](13-cloud-native-standards.md).
