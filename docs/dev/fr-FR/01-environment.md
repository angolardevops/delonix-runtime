...
Voir [02 — Cloner, compiler et tester](02-build-and-test.md) pour la façon d’exécuter chacun d’eux.
...
| **KVM** (`/dev/kvm`) | uniquement pour les microVM — voir [09](09-microvm-setup.md) | `test -w /dev/kvm` |
...
Cloud Hypervisor avec son firmware — voir [09](09-microvm-setup.md). Pour construire des images de VM,
...
Compilez le binaire (voir [02](02-build-and-test.md)) et interrogez-le. Les commandes ci-dessous sont
...
isolez d’abord l’état (voir [Isoler l’état du moteur](02-build-and-test.md#isolating-the-engines-state)).
...
- **Les pièges du firmware des VM et de la construction d’images** (le choix du firmware de Cloud Hypervisor, un `passt` ancien
  dans les builds libguestfs, les permissions de `/boot/vmlinuz-*`) sont traités dans [09](09-microvm-setup.md).
