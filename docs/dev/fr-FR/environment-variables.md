...
| Socket de l’API de nœud | `--addr` > `DELONIX_NODE_API_ADDR` > `unix:///run/delonix-node.sock` | `bins/delonix-node-api-bin/src/main.rs:run` |
...
| `DELONIX_NODE_API_ADDR` | `bins/delonix-node-api-bin/src/main.rs:run` ; transmise par `cmd/serve.rs:run` | Socket de l’API de nœud (`delonix serve node-api`, le contrat `delonix.node.v1` en gRPC et HTTP/JSON). | `unix://<path>`. Défaut `unix:///run/delonix-node.sock`. | `--addr` l’emporte. Uniquement locale (l’uid appelant). |
