...
| `DELONIX_HOSTS_FILE` | `bins/delonix-runtime-bin/src/cmd/hosts_file.rs:hosts_path` | The hosts file that `hosts: [host]` on an `HTTPRoute` writes its managed block into. | A file path; default `/etc/hosts`. | Point it at a scratch file in an isolated run so the real `/etc/hosts` is never touched (writing it needs root). |
