...
| Node API socket | `--addr` > `DELONIX_NODE_API_ADDR` > `unix:///run/delonix-node.sock` | `bins/delonix-node-api-bin/src/main.rs:run` |
...
| VM backend | `--backend` (or the image's `HYPERVISOR`) > `DELONIX_VM_BACKEND` > `defaultProvider` in the providers file (ADR-0054) > `vm default-backend --set` > auto-detection (only with no providers file) | `crates/adapters/delonix-vm/src/lib.rs:standing_backend_choice` |
...
| `DELONIX_NODE_API_ADDR` | `bins/delonix-node-api-bin/src/main.rs:run`; forwarded by `cmd/serve.rs:run` | Socket of the node API (`delonix serve node-api`, the `delonix.node.v1` contract as gRPC and HTTP/JSON). | `unix://<path>`. Default `unix:///run/delonix-node.sock`. | `--addr` wins. Local only (the calling uid). |
...
| `DELONIX_PROVIDERS_CONFIG` | `cmd/providers_config.rs:locate_with` | Path to the node's providers file (ADR-0054), ahead of `$XDG_CONFIG_HOME/delonix/providers.yaml` and `/etc/delonix/providers.yaml`. | A path. | Must name an existing file — never falls back to another one. The first file found is the configuration; files are never merged. A file that cannot be read makes a VM request without `--backend` fail instead of guessing a provider. |
...
| `DELONIX_PROXMOX_PASSWORD_FILE` | `cmd/vmbackends.rs:credential_value` | Path to a file holding that password; preferred over `DELONIX_PROXMOX_PASSWORD`. | A path. | Refused unless only its owner can read it (`chmod 600`). The providers file's `passwordFile` maps onto it. |
...
| `DELONIX_PROXMOX_TEST_MOVE_NODE` | `crates/providers/delonix-proxmox/tests/live.rs` | The node of the same cluster the `vm move` live cases move a VM to (ADR-0053). Needs a lab cluster of two nodes — never production. | Unset: the two move cases skip. | |
| `DELONIX_PROXMOX_TEST_SHARED_STORAGE` | `crates/providers/delonix-proxmox/tests/live.rs` | A storage every node of the cluster shares (NFS, Ceph RBD), where the move cases put the VM's disk — a disk on local storage is refused, not copied. | Unset: the two move cases skip. | |
| `DELONIX_PROXMOX_TEST_CALLBACK_ADDR` | `crates/providers/delonix-proxmox/tests/live.rs:sdn_controllers_fabric_dhcp_and_ip_reservations_round_trip_through_the_node` | The address the NODE can reach this host at: the test starts a stub HTTP server and gives the node `http://<addr>:<port>/…` as the URL of an IPAM and a DNS controller, because the node verifies both by calling them. | An IP the node routes to (`192.168.122.1` for a libvirt-NAT lab node). | Without it the controller half of that test is skipped; the rest still runs. |
