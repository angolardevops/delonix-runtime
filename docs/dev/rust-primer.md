...
policy (LANG-01) and its gate are covered in [Contributing workflow](contributing-workflow.md).
...
(`scripts/contract_gate.py`, `buf`); see [Build and test](build-and-test.md).
...
- **Tests must not touch the host's real state.** Take a temporary directory and pass it in
  ...
  validation described in [Build and test](build-and-test.md).
...
The full list and how to run each one locally is in [Build and test](build-and-test.md).
