...
Related pages: the layers and the reasons behind them in [Architecture](architecture.md),
what each crate holds in [Crates](crates.md), the Rust idioms in
[Rust primer](rust-primer.md), and the workflow (worktree, version, ADRs, PRs) in
...
| **`scripts/arch_fitness.py`** | Layer direction, crate directory = layer, versions only in the root, consumer names, and the debt ratchets listed in [Architecture](architecture.md#layers-and-the-allowed-direction) (see [§4](#4-structure-where-code-goes) and [§5](#5-internal-api-and-boundaries)). | **Enforced (gate)**: CI job `arch`, baseline in `scripts/arch_baseline.json`. |
...
table and current exceptions are in [Architecture](architecture.md#layers-and-the-allowed-direction).
...
This table uses the crates as they exist today. Check [The crates](crates.md) for each crate's contents
...
- **No raw `clone()`/`fork()` in a multi-threaded process** (the tokio servers, the Docker API
  ...
  **Decided**: AGENTS.md § "Auditoria de segurança #3", item 5. Background:
  [Rust primer for this codebase](rust-primer.md#why-forkclone-in-a-multi-threaded-process-is-dangerous).
...
- **Temporary files: use `delonix_state::write_private_temp`.** It opens with a unique name,
  ...
  `crates/adapters/delonix-state/src/store.rs`. **Convention (observed)**: `delonix-sdn/src/bpf.rs`,
...
- **Read–modify–write goes through `update`, never `load` → mutate → `save`.**
  `Store::update` and `JsonStore::update` (`crates/adapters/delonix-state/src/store.rs`)
...
- **Lock files are never deleted.** Deleting one opens a window where two processes lock different
  inodes. **Decided**: doc comment of `store.rs:lock_path` (`delonix-state`).
- **`SecretStore::update` is the one `update` whose lock is best-effort.** Its `FileLock::acquire`
  returns `Option` and proceeds unlocked if the lock file cannot be opened, unlike `Store` and
  `JsonStore`. **Not decided**: whether it should refuse like the others; follow the surrounding
  code and say so in the PR if you touch it.
...
- **Where they go.** Unit tests go in `#[cfg(test)] mod tests` at the bottom of the file. Integration
  ...
  opt-in. **Convention (observed)**; details in [03 § 3.9](rust-primer.md#39-tests).
...
- **Concurrency bugs get a real race.** Use threads plus an explicit sleep inside the critical
  window. **Convention (observed)**: `delonix-state/src/store.rs:jsonstore_update_concorrente_nao_perde_escritas`.
...
- **Doc comments (`///`, `//!`) on public items and module headers** say what the item promises and
  ...
  `delonix-state/src/store.rs:write_private_temp`, `exitcode.rs`. **Not decided**: no `missing_docs` lint is enabled.
...
- **When you move a boundary, update its records in the same change**:
  - an ADR **before** the code, for a new provider, port, daemon, external dependency in an engine
    ...
    [Contribution workflow](contributing-workflow.md#when-to-write-an-adr));
  ...
  - the generated artefacts: `docs/schema/v1/delonix.json`, `docs/api/openapi.yaml`, the CLI
    ...
    [Publishing the documentation](publishing-docs.md)).
