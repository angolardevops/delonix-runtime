# 15. Environment variables (`DELONIX_*`)
...
script and in [02](02-build-and-test.md), not here.
...
| `DELONIX_ALLOW_UNENFORCED_LIMITS` | `bins/delonix-runtime-bin/src/cmd/container.rs:preflight_resource_limits` | Runs a container with `-m`/`--cpus`/`--cpu-weight` even when this session has no cgroup delegation, instead of refusing (exit 69). | Set (any value) → run, unlimited, with a warning. | The kernel never sees the limits. See [01 — cgroup delegation](01-environment.md#cgroup-delegation-some-limits-are-refused-others-are-not-enforced). |
