...
already read [01 — Preparing your environment](01-environment.md) and can build the tree
...
state and has restarted real workloads before (see [02](02-build-and-test.md)). A Cloud Hypervisor
...
[08 — Delonixfile and VMfile](08-delonixfile-and-vmfile.md). Image builds are amd64 only
...
- Implement `VmBackend` in **its own crate** if it talks to a remote API (the engine crate stays free
  ...
  (see [05 — Architecture](05-architecture.md)).
...
- **Pure unit tests** — argv and XML builders are pure functions so they test without a hypervisor:
  ...
  `cargo test -p delonix-runtime-bin vmfile` (see [02](02-build-and-test.md) for `protoc` and the
  target directory).
