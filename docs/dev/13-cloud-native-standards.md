# 13. Cloud native standards, layer by layer
...
Read [4. Cloud native primer](04-cloud-native-primer.md) first if namespaces, cgroups, overlayfs
...
- **OCI artifacts** (single-blob, empty config `application/vnd.oci.empty.v1+json`, the pattern
  ...
  way (see [9. Building microVMs](09-microvm-setup.md)). Annotations are read only *after* the
  digest check.
...
- **Native SDN** (the default for containers): the rootless holder netns, bridge, IPAM, nftables
  firewall — `crates/adapters/delonix-sdn` (see [4.5](04-cloud-native-primer.md) and
...
external suite. See [5. Architecture](05-architecture.md) and [7. System design](07-system-design-interview.md).
...
- **Backends behind a port**: `trait VmBackend` and `register_backend` in `src/lib.rs`. Cloud
  ...
  (`crates/providers/delonix-proxmox`). See [9. Building microVMs](09-microvm-setup.md).
