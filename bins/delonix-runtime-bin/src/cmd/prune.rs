...
mod tests {
    ...
    /// mentions must.
    #[test]
    fn vm_data_nothing_references_is_named_but_never_doomed() {
    ...
    /// An unreadable mtime is *unknown*, never *old*. The rule holds across the
    /// house and here it deletes data: an entry the engine cannot measure must
    /// not be doomed for it.
    #[test]
    fn the_build_cache_expires_by_last_use_and_never_by_ignorance() {
        let day = 24 * 60 * 60;
        let ttl = super::BUILD_CACHE_TTL_DAYS;
        assert!(!super::build_cache_is_stale(Some(0), ttl), "usada agora");
        assert!(
            !super::build_cache_is_stale(Some(ttl * day), ttl),
            "exactamente no limite ainda conta como viva"
        );
        assert!(super::build_cache_is_stale(Some(ttl * day + 1), ttl));
        assert!(
