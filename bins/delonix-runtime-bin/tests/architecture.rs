...
/// Every directory that is really a crate (has a `Cargo.toml`): `crates/<layer>/*`,
/// `bins/*`, and — so a crate dropped in the wrong place is still counted —
/// `crates/*` itself. Where each crate is ALLOWED to live is the job of
/// `scripts/arch_fitness.py`; this gate only needs to see all of them.
///
...
fn crates_on_disk() -> BTreeSet<String> {
    ...
    let root = repo_root();
    let mut candidates = dirs(&root.join("bins"));
    for d in dirs(&root.join("crates")) {
        candidates.push(d.clone()); // a crate placed directly under crates/
        candidates.extend(dirs(&d)); // crates/<layer>/<crate>
    }
